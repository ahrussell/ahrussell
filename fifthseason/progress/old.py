...
class SuffixTree(object):
    ...
    def _add_depths(self):
        for key,edge in self.edges.iteritems():
            self._add_depth(edge, edge.length+2)
    ...
    def _add_depth(self, edge, depth):
        try:
            ...
            if self.nodes[edge.dest_node_index].suffix_node == -1:
                edge.depth = depth - edge.length + 2
                return
            ...
            for new_edge in new_edges:
                self._add_depth(new_edge, depth+new_edge.length+2)
    ...
    def get_patterns(self, length):
        patterns = []
        for x,edge in self.edges.iteritems():
            if edge.depth >= length and self.nodes[edge.dest_node_index].suffix_node == -1:
                start_index = edge.first_char_index-edge.depth
                compressions = edge.depth
                patterns.append((start_index, compressions))
