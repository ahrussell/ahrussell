...
class SuffixTree(object):
    ...
    def _add_depths(self):
        for key,edge in self.edges.iteritems():
            self._add_depth(edge, 1)
                
    def _add_depth(self, edge, depth):
        try:
            edge.depth
            ...
            new_edges = [x for y,x in self.edges.iteritems() if x.source_node_index == edge.dest_node_index]
            for new_edge in new_edges:
                self._add_depth(new_edge, depth+edge.length)
    
    def _add_leaves(self):
        for key,edge in self.edges.iteritems():
            if self.nodes[edge.dest_node_index].suffix_node == -1:
                self.nodes[edge.dest_node_index].left_char = self.string[edge.first_char_index]
    ...
    def get_leaves(self):
        ...
        for node in self.nodes:
            try:
                leaves.append(node.left_char)
            except AttributeError:
                pass
        
        return leaves
                
    # extracts all patterns of length length or higher
    def get_patterns(self, length):
        patterns = []
        for key,edge in self.edges.iteritems():
            node = self.nodes[edge.dest_node_index]
            
            if node.suffix_node == -1:
                
                depth = self.depth(edge.dest_node_index) - edge.length-1
                
                if depth >= length:
                    
                    parent = [x for y,x in self.edges.iteritems() if x.dest_node_index == edge.source_node_index][0]
                    children = [x for y,x in self.edges.iteritems() if x.source_node_index == parent.dest_node_index]
                    
                    '''print depth
                    print self.edge_str(edge)
                    print "parent: "+ self.edge_str(parent)
                    print "children: " + str(len(children))'''
                    
                    maximal = False
                    for child in children:
                        #print self.edge_str(child)
                        if child != edge and self.nodes[child.dest_node_index].suffix_node == -1:
                            maximal = True
                            #print "maximal"
                            break
                    
                    if maximal:
                        start_index = edge.first_char_index-depth
                        end_char = node.left_char
                        
                        patterns.append((start_index, depth, end_char))
                        
                        '''print self.string[start_index:depth+start_index]
                        print (start_index, depth, end_char, edge.dest_node_index)
                        print "-"*60
                        print''' 
    ...
    def depth(self, node_index):
        edges = [x for y,x in self.edges.iteritems() if x.dest_node_index == node_index]
        if edges == []:
            return 1
        
        parent = edges[0]
        
        '''if self.nodes[edges[0].dest_node_index].suffix_node != -1:
            print self.edge_str(edges[0])
            print parent.length+1'''

        return parent.length+1 + self.depth(parent.source_node_index)